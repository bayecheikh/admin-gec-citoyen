...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listmodelescourriers : listmodelescourriers"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listmodelescourriers.length?false:true" 
  ...
  export default {
    ...
    methods: {
      visualiserItem (item) {   
        ...
        this.$router.push('/modelescourriers/detail/'+item.id);
      },
      editItem (item) {
        this.$store.dispatch('modelescourriers/getDetail',item)
       ...
       deleteItem (item) {
        ...
        .then(async (response) => { 
            ...
            this.$store.dispatch('modelescourriers/deletemodelecourrier',this.activeItem.id)
